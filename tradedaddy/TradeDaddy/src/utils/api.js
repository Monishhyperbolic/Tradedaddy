/**
 * TradeDaddy API Client v5
 * Named exports — no bundler tree-shaking issues
 */
...
/* ── Market Data ── */
export const getChart = (symbol, range = '3mo', interval = '1d') =>
  req(`/api/chart/${encodeURIComponent(symbol)}?range=${range}&interval=${interval}`)
export const getQuote = (symbol) =>
  req(`/api/quote/${encodeURIComponent(symbol)}`)
...
export const groqChat = async (messages, system) => {
  ...
  else if (Array.isArray(messages)) built.push(...messages)
